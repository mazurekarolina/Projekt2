...
class Transformacje:
    ...
    def fl2xy(self, ff: float, ll: float, strefa: int) -> tuple:
        f = ff*pi/180
        l = ll*pi/180
    ...
    def u2000(self, f:float, l:float, strefa: int):
        m2000 = 0.999923
        xgk, ygk = self.fl2xy(f, l, strefa)
        l0 = self.get_l0(strefa)
        x = xgk * m2000
    ...
    def get_l0_old(self, l: float, strefa:str) -> int:
        """
    ...
    def stopnie_na_dziesietne(self, stopnie: int, minuty: int, sekundy: float):
        return stopnie + minuty/60 + sekundy/3600

    def u1992(self, f:float, l:float, strefa:int):
        xgk, ygk = self.fl2xy(f, l, strefa)
    ...
    def wgs84(self, f, l, strefa):
        xgk, ygk = self.fl2xy(f,l, strefa)
        if strefa == 1:
            x, y = self.u1992(f, l, strefa)
        else:
            x, y = self.u2000(f, l, strefa)
        return {"x": x, "y": y, "xgk": xgk, "ygk": ygk}
        
    def grs80(self, f, l, strefa):
        xgk, ygk = self.fl2xy(f, l, strefa)
        if strefa == 1:
            x, y = self.u1992(f, l, strefa)
        else:
